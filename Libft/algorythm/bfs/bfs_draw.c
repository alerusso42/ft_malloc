...
/*   Updated: 2025/11/12 19:32:50 by alerusso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	find_shortest_route(t_bfs *bfs, int x, int y)
{
	...
	record = INT_MAX;
	if (bfs->map[y][x + 1].distance[bfs->n] > 0)
		record = bfs->map[y][x + 1].distance[bfs->n];
	if ((bfs->map[y][x - 1].distance[bfs->n] > 0) && \
	(bfs->map[y][x - 1].distance[bfs->n] < record))
		record = bfs->map[y][x - 1].distance[bfs->n];
	if ((bfs->map[y + 1][x].distance[bfs->n] > 0) && \
	(bfs->map[y + 1][x].distance[bfs->n] < record))
		record = bfs->map[y + 1][x].distance[bfs->n];
	if ((bfs->map[y - 1][x].distance[bfs->n] > 0) && \
	(bfs->map[y - 1][x].distance[bfs->n] < record))
		record = bfs->map[y - 1][x].distance[bfs->n];
	if (record == bfs->map[y][x + 1].distance[bfs->n])
		return (1);
	if (record == bfs->map[y][x - 1].distance[bfs->n])
		return (2);
	if (record == bfs->map[y + 1][x].distance[bfs->n])
		return (3);
	if (record == bfs->map[y - 1][x].distance[bfs->n])
		return (4);
	return (-1);
...
static void	update_one(t_bfs *bfs, int *x, int *y, int rec)
{
	if (rec == 1)
	{
		bfs->map[*y][*x + 1].distance[bfs->n] = bfs->mark;
		(*x)++;
	}
	if (rec == 2)
	{
	...
	if (rec == 3)
	{
		bfs->map[*y + 1][*x].distance[bfs->n] = bfs->mark;
		(*y)++;
	}
	if (rec == 4)
	{
...
static void	normalize_path(t_bfs *bfs, int map_x, int map_y)
{
	...
	while (++y != map_y)
	{
		...
		while (++x != map_x)
		{
			if (bfs->map[y][x].distance && bfs->map[y][x].distance[bfs->n] > 0)
